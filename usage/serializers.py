...
class UsageRecordSerializer(serializers.ModelSerializer):
    ...
    def get_api_requests_remaining(self, obj):
        ...
        return max(
            0,
            obj.plan.max_api_requests - obj.api_requests
        )
    ...
    def get_api_requests_percentage(self, obj):
        ...
        percentage = (
            Decimal(obj.api_requests)
            / Decimal(obj.plan.max_api_requests)
        ) * 100

        return round(
            min(percentage, Decimal("100")),
            2
        )
    ...
    def get_projects_remaining(self, obj):
        ...
        return max(
            0,
            obj.plan.max_projects - obj.projects_count
        )
    ...
    def get_projects_percentage(self, obj):
        ...
        percentage = (
            Decimal(obj.projects_count)
            / Decimal(obj.plan.max_projects)
        ) * 100
    ...
    def get_storage_remaining_gb(self, obj):
        ...
        remaining = (
            Decimal(str(obj.plan.storage_limit_gb))
            - obj.storage_used_gb
        )
